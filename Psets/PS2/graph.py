...
class WeightedEdge(Edge):
    ...
    def __str__(self):
        # MY_CODE
        return super().__str__() + ' ({}, {})'.format(self.total_dist,
                                                      self.outdoor_dist)
...
class Digraph(object):
    ...
    def add_edge(self, edge):
        ...
        in the  graph."""
        # MY_CODE
        if (edge.get_source() not in self.nodes) or \
                (edge.get_destination() not in self.nodes):
            raise ValueError('Nodes in edge not present')
        else:
